...
export const createStore = () => {
  const atomStateMap = new WeakMap<AnyAtom, AtomState>() // 存储value
  const mountedMap = new WeakMap<AnyAtom, Mounted>() // 存储依赖
  ...
  const updateDependencies = <Value>(
    ...
    // 当依赖的state或者size发生变化时，就更新d
    if (changed || nextAtomState.d.size !== dependencies.size) {
      nextAtomState.d = dependencies
    }
  }

  // 每次value改变，都会产生新的state
  ...
  // 读取state的值，
  // 1: 如果state存在，
  //   1.1: 确保所依赖的atom是最新的，如果依赖是新值或者没有mount，递归调用readAtomState
  // 2: 调用getter获取依赖，并添加到atom的d<dependency, state>
  ...
  // 如果有有listener、dependent的时候就可以卸载
  ...
  const recomputeDependents = (atom: AnyAtom): void => {
    const dependencyMap = new Map<AnyAtom, Set<AnyAtom>>() // 记录atom与它依赖的Atom的关系，<atom, [所有它依赖的atom的集合]>
    // 如果dirtyMap对应的数量大于1 表示，存在依赖D同事D依赖A的情况（相当于父级的个数）
    const dirtyMap = new WeakMap<AnyAtom, number>() // 记录dependent出现的次数，表示它依赖了多少个其他的atom，相当于dependent依赖的atom的数量
    ...
    const loop2 = (a: AnyAtom) => {
      ...
      mounted?.t.forEach((dependent) => {
        if (dependent !== a) {
          ...
          // 如果本次是dependent最后一次依赖的循环，就更新dependent的值
          if (!dirtyCount) {
            // 如果还依赖了其他的Atom，就是
            let isChanged = !!dependencyMap.get(dependent)?.size // 
            if (isChanged) {
              const prevAtomState = getAtomState(dependent) // 获取state
              const nextAtomState = readAtomState(dependent) // 重新从父级开始获取state，会重新整理依赖，并调用getter从依赖的atom中获取最新值
              isChanged =
                !prevAtomState ||
                !isEqualAtomValue(prevAtomState, nextAtomState)
            }
            if (!isChanged) {
              // 如果dependent的值没有改变，就从依赖的它的atom的依赖中移除dependent
              // 如果dependent没有被任何的atom依赖了，就从集合中删除
      ...
      atomState.d.forEach((_, a) => {
        if (a !== atom) {
          ...
          if (mounted) {
            ...
            // 如果 a 没有被任何atom依赖，并且没有listener的话表示可以被删除
